...
export const getUserData = asyncHandler(async (req, res, next) => {
  if (req.user.phone) {
    req.user.phone = CryptoJS.AES.decrypt(
      req.user.phone,
...
export const getFavoriteProducts = asyncHandler(async (req, res, next) => {
  const userId = req.user._id;
  ...
  return res.status(StatusCodes.OK).json({
    message: "Done",
    "Favorite Products❤️": result.favoriteProducts,
  });
});

export const getAllRegularUsers = asyncHandler(async (req, res, next) => {
  const users = await userModel
    .find({ role: "user" })
    .select("-role -password -confirmCode");
